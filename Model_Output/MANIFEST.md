...
**Current pipeline state (as of 2026-08-10):** market panel filtered to
>=70% real coverage per market (**834 tomato / 809 onion / 82 potato** —
see below), potato zones P1-P3 relocated to Darjeeling/Diamond
Harbour/Dehradun with real climate+satellite data. The panel has moved
**four more times since the 2026-08-01 grid fix** (policy-events
verification + `PANEL_END` extension, WPI/diesel-LPG vintage refresh,
macro long-history extension, `s2_ndvi_anom` leakage fix — each documented
below) — any output dated before 2026-08-04 should be treated as stale
relative to the current pipeline, not just anything before 2026-08-01.
...
| `15_Ablation_Study_M0_M4.py` | `ablation_raw_results.csv`, `ablation_predictions.csv`, `table_ablation.csv`, `table_mase.csv`, `fig_ablation_*.png` | 2026-08-04 | 🟢 | Re-run on the grid-fixed panel (834/809/82 markets). Full M0-M6 + inline B1_Naive. Monotonic constraint on export_banned/export_duty_pct/mep_usd_per_tonne (added 2026-07-31) retained — negligible accuracy cost. **Retrained again 2026-08-04** for the `s2_ndvi_anom` leakage fix — net accuracy impact small and mixed (e.g. onion h=13w MAPE 40.9%→35.8%), no ranking changes. Has two run modes via `MARKET_LEVEL_DIAGNOSTIC`; resting state `False`. |
...
| `38_SDID_InSpace_Placebo_Test.py` | `table_sdid_inspace_placebo.csv`, `fig_sdid_inspace_placebo.png` | 2026-08-01 | 🟢 | Standard synthetic-control falsification test (Abadie, Diamond & Hainmueller 2010): every individual tomato/potato market treated as a placebo unit, one at a time, against the same donor pool. Real onion postban ATT judged against the resulting null distribution: p=0.131 — does not clear significance against this stronger test either, consistent with Part C's jackknife-CI finding above. |
| `39_SDID_Stacked_MultiEpisode_EventStudy.py` | `table_sdid_stacked_event_study.csv`, `table_sdid_stacked_summary.csv` | 2026-08-01 | 🟢 | Extends the single-episode (2023-24) SDID design to all 3 verified onion export bans in the panel window (Sep 2019, Sep 2020, 2023-24), addressing the underlying n=1 identification problem by pooling 3 independent episodes into one event study. |
...
| Script | Outputs | Last generated | Notes |
|---|---|---|---|
| `40_Escalation_Signature_Head.py` | `table_escalation_signature_loeo_percrop.csv`, `table_escalation_signature_scores_percrop.csv`, `table_escalation_signature_placebo_test.csv`, `fig_escalation_signature_percrop_onion.png`, `fig_escalation_signature_placebo_test.png` | 2026-08-08 | Per-crop LightGBM early-warning classifier, validated with leave-one-episode-out CV and a placebo-in-time significance test. Onion 2019/2023 validate cleanly (AUC 0.93/0.89, p=0.000); onion 2020 does not (AUC 0.68, p=0.660) — a real, different-shaped price path, confirmed via direct feature inspection, not a labeling artifact. Deliberately not iterated further past this point (data-volume limited — 3 onion, 1 tomato, 1 potato episodes total). |
...
- Scripts 12, 13, 15b, 15c, 17, 18b, 26 — deferred re-runs. These now predate not just the 2026-08-01 grid fix but every fix since (policy/PANEL_END, WPI vintage, macro long-history, `s2_ndvi_anom` leakage) — more stale than a single "grid fix" label suggests.
...
- Manuscript: 3 of an estimated 6 sections drafted — see README §9.
