...
| Script | Outputs | Last generated | Notes |
|---|---|---|---|
| `40_Escalation_Signature_Head.py` | `table_escalation_signature_loeo_percrop.csv`, `table_escalation_signature_scores_percrop.csv`, `table_escalation_signature_placebo_test.csv`, `fig_escalation_signature_percrop_heldout.png`, `fig_escalation_signature_placebo_test.png` | 2026-09-02 (revised) | **Revised same day: fixed a real LOEO train/test overlap (independent audit finding, confirmed).** The padding negatives added to each fold's test window (near the event, `label==0`) satisfied the exact same condition as `train_mask` (`episode != ep['name']`) — they were trained AND tested on, inflating every reported AUC/AP. Fixed by excluding those rows from `train_mask` too. Mean held-out AUC after the fix: tomato 0.782 (was 0.801), onion 0.805 (was 0.831), potato 0.810 (was 0.849) — a modest, expected downward correction, not a collapse; the qualitative "genuine per-crop discrimination" story holds. Per-episode AUC/AP in the current `table_escalation_signature_loeo_percrop.csv`. **The placebo-in-time test (Section 5) reuses these now-fixed models and its p-values shifted noticeably as a side effect (e.g. onion_2020 national: 0.660→0.085) — but Section 5 has its OWN separate, still-unfixed issue** (the placebo candidate pool is scored mostly in-sample, only the real episode's window is genuinely held out — see the independent audit's second Script 40 finding). Do not treat the new placebo p-values as validated until that's fixed too; only the LOEO AUC/AP numbers above are trustworthy as of this revision. |
| `41_Escalation_Signature_Nashik_Hub_Test.py` | `table_escalation_signature_nashik_loeo.csv`, `table_escalation_signature_nashik_placebo.csv`, `fig_escalation_signature_nashik_vs_national.png` | 2026-09-02 (revised) | **Revised same day, same fix as Script 40** (identical LOEO train/test overlap bug, inherited — same fix applied). Nashik-hub LOEO AUC after the fix: onion_2019 0.809, onion_2020 0.787, onion_2023 0.947. **The previously-reported "2020's placebo p falls tenfold (0.660→0.064)" headline is now STALE** — after just the LOEO fix, the placebo p-value for onion_2020 moved to 0.000 (Nashik-hub) vs. 0.085 (national, Script 40) — a much larger swing than the fix alone should produce, which is the expected symptom of Section 5's separate unfixed in-sample-scoring issue (see Script 40's row above) amplifying a now-different classifier's scores. **Do not cite either the old (0.064) or new (0.000) p-value as a validated localization result until Section 5's placebo-scoring methodology is also fixed** — the honest current state is "LOEO discrimination confirmed clean, localization hypothesis's significance test still needs a real fix before its p-value means anything." |
