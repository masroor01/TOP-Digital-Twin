...
- **NEW 2026-09-13, not fixed: the same class of skew likely also exists in the production React/Node dashboard's inference path** (`web/backend/src/models/index.js`, the one actually deployed at topdigitaltwin.micskuast.in). Its `verify.mjs` fixture suite claims to verify "NaN/missing-value routing," but `web/generate_js_models.py` builds its Python ground-truth predictions by sampling real rows from `reference_rows.csv` and calling `model.predict(X)` directly, with **no `fillna(0)`** applied (line ~76) — the same rows confirmed to carry real NaN in feature columns above. So the verification only confirms the JS's native NaN-routing matches *Python's own native NaN-routing*, never the *trained* expectation (every training row was `fillna(0)`'d before the model ever saw it) — meaning this backend was likely never actually serving what the model was trained on, for exactly the markets Script 24's bug affected. Not fixed here: touching it means patching `toNumeric()`/`buildInput()` in `models/index.js`, then regenerating fixtures and re-running `verify.mjs` against `X.fillna(0)`-based ground truth before trusting it — a bigger, separate task, and this is the live production dashboard, not a dev tool.
