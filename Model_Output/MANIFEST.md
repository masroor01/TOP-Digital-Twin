...
- **NEW 2026-09-11, not fixed: `geocode_cache.json` has wrong coordinates for two Tamil Nadu districts** — "Thirunelveli" and "Thirupur" both resolve to ~13°N/80°E (near Chennai) instead of their real locations (Tirunelveli ~8.7°N near Kanyakumari; Tiruppur ~11.1°N near Coimbatore). Found via the M7 drought-layer cross-check (IDM's nearest-grid-point search collided both districts onto the same cell, which was geographically implausible), then confirmed against Script 16's actual `zone_assignment.csv`: 13 real tomato/onion markets in these two districts (Palayamkottai, Melapalayam, Tiruppur North/South, Dharapuram, Kangayam, etc.) are assigned to Andhra Pradesh zones — Madanapalle (189km) for tomato, Kurnool (392-395km) for onion — meaning their M3/M4 climate/satellite features have been wrong for as long as zone assignment has been in production. Root cause not yet traced (likely a Nominatim mismatch on the misspelled district names, defaulting to a state-level or wrong-city centroid). Fixing this means correcting `geocode_cache.json` and re-running Script 16 (regenerates `market_zone_features.csv`/`crop_weekly_features.csv`) and every downstream script that consumes them (14→16→22, 15, 23) — a real, separate task, not done as part of the drought-layer work that surfaced it.
