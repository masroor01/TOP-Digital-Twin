...
- ~~The same class of skew likely also exists in the production React/Node dashboard's inference path~~ — **fixed 2026-09-13** (commit `7772fed`): `toNumeric()` in `web/backend/src/models/index.js` returned `NaN` for a missing/non-numeric value (fed straight into the m2cgen tree code's native NaN-routing, never exercised at training time), now returns `0`. Confirmed the bug was live and pervasive first: every one of 1,737 rows in `web/data/production_models/reference_rows.csv` (the file the deployed backend actually reads — confirmed byte-identical to the `Model_Output/` copy) has at least one NaN feature. Also fixed the verification suite itself, which had the identical gap and would not have caught this: `web/generate_js_models.py` built its Python "ground truth" via `model.predict(X)` on X still carrying real NaN, so `verify.mjs` only ever confirmed JS-native-NaN-routing matched Python-native-NaN-routing, never the trained (`fillna(0)`) convention. Regenerated all 12 fixtures with the corrected ground truth and re-ran `verify.mjs`: all 12 models pass at `max_diff=0.000e+0`. The 12 model `.js` files themselves are unchanged (confirmed via `git diff`) — the trained tree logic never needed to change, only how inputs are prepared for it.
