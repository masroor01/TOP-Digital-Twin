...
| `31_Synthetic_DID_Policy_Effect.py` | `table_sdid_policy_effect.csv`, `table_sdid_unit_weights.csv`, `fig_sdid_treated_vs_synthetic.png`, `table_sdid_hub_policy_effect.csv`, `fig_sdid_hub_vs_nonhub.png` | 2026-08-01 | 🟢 | New 2026-08-01: estimates the 2023-24 onion export-restriction episode's price effect directly via Synthetic DID (Arkhangelsky et al. 2021), since Script 30's forecasting model can't recover a reliable magnitude (identification problem — duty/MEP/ban only ever moved together once). Part A (cross-crop, tomato/potato as donors): escalation-phase ATT +56.2% is credible (huge gap vs tomato placebo -30.7%), but the post-ban ATT (+14.5%) **fails its own placebo check** — indistinguishable from same-window tomato (+14.5%) and potato (+15.2%) placebo ATTs, meaning a shared cross-crop shock swamps whatever ban-specific effect exists in this design. Part B (within-onion, Nashik export-hub vs non-hub markets, sidesteps the cross-crop confound): real 2023 postban ATT +3.7% vs a placebo-in-time (fake 2021 dates) ATT of -3.9% — small and inconclusive either way, plausibly because an export ban is a nationally-uniform supply shock, not a regionally-concentrated one. **Honest conclusion: no design tried here reliably isolates the ban's causal magnitude with the available data** — state this directly rather than citing either ATT as a validated effect size. |
