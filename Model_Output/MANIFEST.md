...
| `22_Master_Panel_Join.py` | `data/master_weekly_panel_all_layers.csv` (gitignored, 696MB — regenerate, don't commit) | 2026-09-02 (re-run) | 🟢 | Re-run after the Script 20/21 truncation fixes above and its own `PANEL_END` widening (2030-12-31). All 6 layer joins verified fan-out-free (522,082 rows in → 522,082 out at every step, via the script's own `checked_merge` assertions). Missing-value rates per layer: macro 7.6%, climate/satellite 13.8%, wages 13.3%, cold storage 0%, roads 0%, policy 0%. Final: 522,082 rows x 62 columns. |
