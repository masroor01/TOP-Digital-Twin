          ...
          <table className="w-full text-xs min-w-[480px]">
            ...
            <tbody>
              {cols.map((c) => {
                ...
                return (
                  <tr key={c} className={`border-b border-slate-100 last:border-0 ${changed ? 'bg-amber-50' : ''}`}>
                    <td className="py-1 pr-3 font-mono text-slate-600">{c}</td>
                    <td className="py-1 pr-3 font-mono text-slate-800">{String(baseRow[c])}</td>
                    <td className={`py-1 font-mono ${changed ? 'text-amber-700 font-semibold' : 'text-slate-300'}`}>
