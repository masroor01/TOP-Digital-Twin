...
| `47_Market_Level_Accuracy.py` | The dashboard's "Model Accuracy" KPI is a crop+horizon-level statistic (one shared model per crop/horizon serves every market, so it's the same figure regardless of which market is selected — not a bug, but easy to mistake for one). This computes a genuine per-market MAPE from real per-market backtest predictions (M6 variant), so the React dashboard can show a specific market's own track record alongside the crop-wide average. Cells with fewer than 10 backtested weeks are flagged as insufficient rather than shown. **~1 min.** | `Model_Output/dm_market_level_predictions.csv` |
...
- `table_market_level_accuracy.csv` — Script 47's per-(crop, market, horizon) MAPE from real backtest predictions, feeding the dashboard's per-market accuracy figure
