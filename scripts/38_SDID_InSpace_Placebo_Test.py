...
# Market-name-vs-market_id collision fix (2026-08-14 pipeline-wide bug, applied here
# 2026-08-20): a handful of market NAMES repeat across different states within the
# same crop (e.g. "Fatehabad APMC" in both Haryana and Uttar Pradesh). This script
# groups/pivots on (crop, market) NAME, which would silently collapse those into one
# column. Relabel colliding names to include state, once, right after loading --
# every downstream 'market' string is then genuinely unique with zero change to the
# actual SDID estimation logic.
