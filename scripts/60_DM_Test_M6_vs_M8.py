# -*- coding: utf-8 -*-
"""
Script 60 — Market-Level Diebold-Mariano Test: M6 vs M8 (Fertilizer MRP)
================================================================
Formal significance test of the M8 (fertilizer MRP) layer added via
Scripts 57/59, mirroring Script 55's exact methodology (itself mirroring
Script 18b) but for M6 (pre-fertilizer full model, excluding the
unproduction-wired M7 drought layer) vs M8 (M6 + Urea/DAP/MOP MRP)
instead of M6 vs M7.

Unlike M7, M8 does not have a fold-1-predates-the-data problem: fertilizer
MRP has real training coverage in every one of the 5 CV folds (41-89
months, Jan-2018 onward -- checked directly before running the ablation,
see Script 15's docstring). CPI-AL/RL (acquired alongside fertilizer in
Scripts 58/59) is NOT part of M8 here -- it has essentially zero real
training signal in 4 of 5 folds (only exists from Jun-2025), the same
problem M7's fold 1 had but affecting nearly every fold instead of one;
deferred until it has 2-3 years of history, same standard M7 was held to.

Input:
  Model_Output/dm_market_level_predictions.csv   (from Script 15 with
                                                   MARKET_LEVEL_DIAGNOSTIC=True,
                                                   DIAGNOSTIC_PAIR=('M6','M8'))

Outputs (Model_Output/):
  table_dm_m6_vs_m8_detail.csv    per-market DM stat/p-value
  table_dm_m6_vs_m8_summary.csv   per crop×horizon: % significant,
                                   direction breakdown, Fisher combined p

Run: python scripts/60_DM_Test_M6_vs_M8.py
"""
...
ALPHA    = 0.05
MIN_OBS  = 30
BASELINE, RICHER = 'M6', 'M8'   # must match Script 15's DIAGNOSTIC_PAIR


def diebold_mariano_test(y_true, pred_baseline, pred_richer, h):
    """Identical to Script 18b/55's DM test -- squared-error loss, HLN
...
print('=' * 65)
print('SCRIPT 60: MARKET-LEVEL DIEBOLD-MARIANO TEST -- M6 vs M8 (FERTILIZER MRP)')
print('=' * 65)
...
# Fold-level sanity check (this is the check that caught M7's real problem
# -- fold 1's M6/M7 predictions were byte-identical because IDM/VEDAS both
# postdate fold 1's training cutoff). Fertilizer MRP has real coverage in
# every fold, so this is expected to come back clean, but checked directly
# rather than assumed.
print('[0] Fold-level identity check (per-fold, are M6 and M8 predictions distinct?) ...')
...
detail_path = os.path.join(OUT_DIR, 'table_dm_m6_vs_m8_detail.csv')
...
summary_path = os.path.join(OUT_DIR, 'table_dm_m6_vs_m8_summary.csv')
...
print(f'    every fold, M8 genuinely trained on real fertilizer signal throughout')
print(f'    (unlike M7\'s fold-1 problem) -- per-market counts below are trustworthy')
print(f'    subject to the usual market-non-independence caveat.')
...
print(f'    the fertilizer layer has no reliable market-level effect.')
